...
Antes (t030 original) cada corrida hacía overwrite sobre TODO el path
...
Uso standalone (para probarlo suelto antes de envolverlo en el DAG de Airflow):
    spark-submit --packages org.apache.hadoop:hadoop-aws:3.3.4 src/spark/ingest_credit_risk.py
...
def ingest(spark: SparkSession, csv_path: str, ingestion_date: str) -> None:
    ...
    En particular, el outlier extremo de `income` que se detectó en
    t043 (y que se trata en t041) NO se toca aquí a propósito: Bronze
    debe reflejar exactamente lo que vino de la fuente, sin decisiones
    de limpieza todavía.
    """
