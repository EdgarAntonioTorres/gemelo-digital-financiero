...
def ingest(
    ...
    dimensiones (más abajo), y es deliberadamente no-bloqueante. Las
    columnas de trazabilidad son la única metadata agregada aquí (ver
    docstring del módulo, `t036`), y son deliberadamente aditivas: no
    tocan ni reinterpretan ninguna columna original del dataset.
    """
    ...
    # documenta este dataset como 3,000 filas x 25 columnas. Si cambia,
    # no queremos frenar la ingesta — solo dejar un WARNING visible.
...
def main() -> None:
    ...
    try:
        ...
        logger.exception(
            "Falló la ingesta de Personal Finance Tracker a Bronze por un error inesperado."
        )
        sys.exit(1)
    finally:
